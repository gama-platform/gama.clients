...
class TestStepAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_step_sync(self):
        ...
        self.future_command1 = Future()
        await self.client.step_async(exp_id, sync=True)
        step_res = await self.wait_for_future(self.future_command1)
        self.assertEqual(step_res["type"], MessageTypes.CommandExecutedSuccessfully.value)

        self.future_command1 = Future()
        await self.client.expression_async(exp_id, "cycle")
        expression_val = await self.wait_for_future(self.future_command1)
